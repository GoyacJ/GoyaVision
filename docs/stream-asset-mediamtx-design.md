...
- **docs/requirements.md**：3.1.2 中“添加资产 - 流媒体接入”已统一为“必须接入 MediaMTX”的原则与两种入口。
...
本设计已收敛并内嵌业务、技术与规范评估结论：path_name 生成规则（2.2.1）、创建失败回滚（2.1、2.3）、删除媒体源策略（2.1、3.3）、推流地址返回（2.3）、录制与点播参数约定（2.4）、错误码约定（5.3）、分层与实现顺序（6.1、6.2）。
